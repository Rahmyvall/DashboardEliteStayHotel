...
{
    /**
     * Dashboard Admin, Resepsionis, Pelayanan
     */
    public function index()
    {
        ...
        return view('pages.dashboard', [
            ...
            'pelayananCount' => User::where('role', 'pelayanan')->count(),
    ...
    public function getReservasiLineChart()
    {
        ...
        if ($periode === 'daily') {
            ...
            $data = Reservasi::selectRaw("
                    ...
                    SUM(CASE WHEN status_reservasi = 'checkin' THEN 1 ELSE 0 END) as checkin
                ")
                ->where('tanggal_pesan', '>=', Carbon::now()->subDays(6))
                ->groupBy('tanggal')
                ->orderBy('tanggal')
                ->get();
            ...
            $labels = $data->pluck('bulan')
                ->map(fn($bulan) => Carbon::parse($bulan . '-01')->format('M Y'));
        }
